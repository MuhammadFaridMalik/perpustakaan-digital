@extends('layouts.app')
@section('title', 'Data Master — author')
    ...
    <div x-data="{ showAdd: false, editId: null }">
        ...
        @if ($authors->isEmpty())
            <div class="border border-dashed border-border rounded p-10 text-center text-muted text-sm">
                Belum ada author. Tambahkan minimal 1 sebelum membuat buku.
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-border">
                        @foreach ($authors as $author)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-muted">{{ $author->books_count }}</td>
                                <td class="px-4 py-3 text-right space-x-3">
                                    <button @click="editId = {{ $author->id }}" class="text-sm text-primary hover:underline">Ubah</button>
                            ...
                            <!-- Modal Ubah -->
                            <div x-show="editId === {{ $author->id }}" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4">
                                ...
                                <div class="relative bg-surface rounded border border-border p-6 w-full max-w-sm">
                                    <h4 class="font-medium mb-4">Ubah author</h4>
                                    <form method="POST" action="{{ route('authors.update', $author) }}" class="space-y-3">
                                        @csrf
                                        @method('PUT')
        ...
        <div x-show="showAdd" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <div class="relative bg-surface rounded border border-border p-6 w-full max-w-sm">
                <h4 class="font-medium mb-4">Tambah author</h4>
                @if ($errors->any())
                    <div class="mb-3 text-sm text-danger">{{ $errors->first() }}</div>
                @endif
                <form method="POST" action="{{ route('authors.store') }}" class="space-y-3">
                    @csrf
